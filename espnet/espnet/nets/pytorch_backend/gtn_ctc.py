...
class GTNCTCLossFunction(torch.autograd.Function):
    ...
    def forward(ctx, log_probs, targets, ilens, blank_idx=0, reduction="none"):
        ...
        def process(b):
            ...
            best_path = gtn.best_path(gtn.intersect(g_emissions, g_criterion))
            token_ids[b] = torch.tensor(best_path.labels_to_list(), dtype=torch.long)

            with open('/home/aa.txt', 'a') as file:
                print(f"Token IDs for batch {b}: {best_path.labels_to_list()}", file=file)
            ...
            # temp_g_criterion[b] = g_criterion
        ...
        return torch.mean(loss.cuda() if log_probs.is_cuda else loss), token_ids
        # return torch.mean(loss.cuda() if log_probs.is_cuda else loss)
        # return torch.mean(loss.cuda() if log_probs.is_cuda else loss), (
        #     log_probs.cuda(), 
        #     emissions_graphs.cuda(), 
        #     scales.cuda(), 
        #     temp_g_criterion.cuda(), 
        #     losses.cuda())
