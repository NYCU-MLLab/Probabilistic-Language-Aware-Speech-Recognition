        ...
        if (
            ...
            if(args.freeze_param):                
                if(args.freeze_param[0] == 'whisper_pe'):
                    ...
                    for k, p in model.named_parameters():
                        if k.find('adapter') ==-1 and k.find('lora') ==-1:
                            logging.info(f"Setting {k}.requires_grad = False")
                            p.requires_grad = False
                    ...
                    for k, p in model.named_parameters():
                        if not (k.find('encoder') !=-1 and (k.find('adapter') !=-1 or k.find('lora') !=-1)):
                            logging.info(f"Setting {k}.requires_grad = False")
                            p.requires_grad = False
                    ...
                    for k, p in model.named_parameters():
                        if (k.find('encoder') !=-1 and  (k.find('adapter') ==-1 or k.find('lora') ==-1)):
                            logging.info(f"Setting {k}.requires_grad = False")
                            p.requires_grad = False
                        if (k.find('decoder') != -1):
                            logging.info(f"Setting {k}.requires_grad = False")
                            p.requires_grad = False      
                elif(args.freeze_param[0] == 'only_encoder'):
                    for k, p in model.named_parameters():
                        if (k.find('encoder') == -1):
                            logging.info(f"Setting {k}.requires_grad = False")
                            p.requires_grad = False
                elif(args.freeze_param[0] == 'train-ln'):
                    for k, p in model.named_parameters():
                        if (k.find('ln') == -1 and k.find('lora') == -1):
                            logging.info(f"Setting {k}.requires_grad = False")
                            p.requires_grad = False
            else:
